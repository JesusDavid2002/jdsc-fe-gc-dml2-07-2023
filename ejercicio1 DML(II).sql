...
SELECT AVG(precio) AS precio_media FROM articulos;
...
SELECT COUNT(precio) AS numero_articulos FROM articulos
WHERE precio >= 180;
...
SELECT nombre, precio AS numero_articulos FROM articulos
WHERE precio >= 180
...
SELECT fabricantes.NOMBRE AS Proveedor, MAX(articulos.PRECIO) AS PrecioMasAlto FROM articulos 
JOIN fabricantes 
ON articulos.FABRICANTE = fabricantes .CODIGO
GROUP BY fabricantes.NOMBRE;
